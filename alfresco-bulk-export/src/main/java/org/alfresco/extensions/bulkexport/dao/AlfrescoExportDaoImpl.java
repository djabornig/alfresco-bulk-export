...
public class AlfrescoExportDaoImpl implements AlfrescoExportDao {
    ...
    private QName[] ignoredType = { ContentModel.TYPE_SYSTEM_FOLDER, ContentModel.TYPE_LINK, ContentModel.TYPE_RATING,
            ActionModel.TYPE_ACTION, ActionModel.TYPE_COMPOSITE_ACTION };
    ...
    public String getType(NodeRef nodeRef) throws Exception {
        ...
        return Objects.requireNonNull(this.getQnameStringFormat(qName), "Type must not be null");
    }
    ...
    private boolean isPropertyIgnored(QName qName) {
        ...
        String prefix = getQnameStringFormat(qName);
        if (prefix == null) {
            return true;
        }
     ...
     * @return {@link String}
     */
    private String getQnameStringFormat(QName qName) {
        try {
            qName = qName.getPrefixedQName(nsR);
            return qName.getPrefixString();
        } catch (NamespaceException e) {
            log.warn("Failed to resolve prefix for " + qName);
            return null;
        }
    }
